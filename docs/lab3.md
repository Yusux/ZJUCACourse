...
![上板结果](img/3_nexys_result.png){.center}
    ...
    -   本次实验的测试指令序列非常长，出现 bug 以后较难定位从什么地方开始出错。你可以使用下述方式来快速定位出现问题的 PC。加动态分支预测和预测不 taken 相比，就是在预测跳转并且成功的时候，可以少掉 flush 一个周期的时间开销；或者有的时候会因为预测跳转但是失败，而比预测不 taken 多 flush 一次。将 is_flushed 的 cycle 去掉，两者的 PC 变化应当是一致的。
    ```verilog
    `timescale 1ns / 1ps
    ...
    你可以先用不加分支预测的版本跑一遍仿真，得到一个 trace，再跑你的加了分支预测的版本，得到一个 trace，然后打开 vscode，对比两者的差异。下面是一个写坏掉了的动态分支预测和不加的比较结果：
